...
A LangGraph **stub** is wired at `POST /api/v1/projects/{project_id}/agents/run` (after register/login so the `access_token` cookie is set). It only checks that the project exists and returns a placeholder; it does not call OpenAI. `OPENAI_API_KEY` in `.env` is optional until Requirement Analyzer / Sprint Planner / Risk Analyzer land. The Kanban UI comes next alongside those agents.
