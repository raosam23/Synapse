...
`POST /api/v1/projects/{project_id}/agents/run` (after register/login so the `access_token` cookie is set) runs the Requirement Analyzer: it writes `ai_opinion` on the project and creates backlog tasks (`sprint_id` null). Tests mock the LLM. A live run needs `OPENAI_API_KEY` in `.env`. Sprint Planner and Risk Analyzer are still later. The Kanban UI comes next alongside those agents.
